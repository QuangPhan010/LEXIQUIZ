...
const GamePlay: React.FC = () => {
      ...
      <div className="min-h-screen bg-slate-50 text-slate-900 selection:bg-primary-100">
        ...
        <main className="max-w-md mx-auto pt-24 px-6 text-center">
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-xl shadow-slate-200/50 mb-10 relative animate-pulse">
            ...
            <h1 className="text-2xl font-black mb-1 tracking-tight">You're In!</h1>
            <p className="text-slate-500 text-sm font-bold">Waiting for the host to start the battle...</p>
          </div>

          <Card className="p-6 border-slate-200 bg-white/80 backdrop-blur-xl">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Joined Warriors ({players.filter(p => !p.is_host).length})</p>
        ...
        <main className="max-w-4xl mx-auto pt-20 px-4 pb-12">
          ...
          {answered ? (
            <div className="text-center py-20 bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-200 animate-in zoom-in duration-500">
              ...
              <h3 className="text-2xl font-black mb-1">Answer Received</h3>
              <p className="text-slate-400 text-sm font-medium">Wait for the reveal!</p>
            </div>
          ) : (
              ...
              {currentQuestion.choices.map((choice: any, idx: number) => {
                ...
                return (
                  <button
                    ...
                    className={`relative group h-24 rounded-[1.5rem] p-0.5 overflow-hidden transition-all hover:translate-y-[-2px] active:translate-y-[1px] ${colors[idx % 4]} border-b-4`}
                  >
                    <div className="h-full w-full bg-white/10 flex items-center px-6">
                      ...
                      <span className="text-lg font-black text-white text-left leading-tight">{choice.text}</span>
                    </div>
                  </button>
                );
              })}
  ...
  if (gameState === 'reveal') {
    const isCorrect = currentQuestion?.choices.find((c: any) => c.id === selectedChoiceId)?.is_correct;
      ...
      <div className={`min-h-screen text-white flex flex-col ${isCorrect ? 'bg-emerald-500' : 'bg-rose-500'} transition-colors duration-700`}>
        ...
        <main className="flex-1 flex flex-col items-center justify-center px-4 py-12">
          <div className="text-center animate-in zoom-in duration-500">
            <div className="inline-flex p-8 rounded-[2.5rem] bg-white/20 backdrop-blur-xl mb-8 shadow-xl">
              {isCorrect ? <Zap className="h-16 w-16 fill-current" /> : <Skull className="h-16 w-16" />}
            </div>
            <h1 className="text-5xl font-black mb-2 tracking-tight">{isCorrect ? 'AWESOME!' : 'Ouch!'}</h1>
            <p className="text-xl font-bold opacity-80 mb-10">{isCorrect ? 'Keep it up!' : 'Focus for the next one!'}</p>
            
            <div className="bg-white/10 backdrop-blur-md rounded-[2rem] p-6 inline-block px-10 border border-white/20">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] mb-1">Total Score</p>
        ...
        <div className="bg-black/10 backdrop-blur-2xl p-6">
          <div className="max-w-md mx-auto flex items-center justify-center space-x-3 text-white/70">
            ...
            <span className="text-xs font-black uppercase tracking-widest">Next Question Loading...</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-slate-50 text-slate-900 selection:bg-primary-100">
        ...
        <main className="max-w-md mx-auto pt-24 px-6 pb-12 text-center">
          <div className="mb-10 animate-in slide-in-from-top-8 duration-700">
            ...
            <h1 className="text-3xl font-black tracking-tight mb-1">{myRank === 1 ? 'VICTORY!' : 'Finished!'}</h1>
            <p className="text-slate-400 font-medium text-sm">You did your best!</p>
          </div>
